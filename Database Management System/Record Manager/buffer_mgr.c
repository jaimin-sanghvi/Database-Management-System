...
/*			printf("\n pageNumber in pin page : %d \n", pageNum);*/
			pos=-1;
			if (pos == -1){
				readBlock(pageNum, stat->fh, ph);
				ensureCapacity(pageNum, stat->fh);
				page->data = ph;
				/*int k;
				for (k = 0; k < 12; k++)
					printf("%c", page->data[k]);*/
...
RC unpinPage(BM_BufferPool *const bm, BM_PageHandle *const page){	
   ...
   struct CLOCK *buf_clock;
	...
	case RS_FIFO:
		...
		/*printf("\n\n...Unpin : Before writing a  block, pageNumber is .... %d\n\n", page->pageNum );*/
		writeBlock(page->pageNum, stat->fh,page->data);
	/*	writeBlock(pageNumber, stat->fh,page->data);*/
		break;
	case RS_LRU:
		pos = searchLRUFramePosition(page->pageNum);
		buf_lru = start_lru;
